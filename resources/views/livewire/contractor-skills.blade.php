<div class="py-2 px-8 pb-2 md:w-full">

    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">

        <div class="flex justify-between">
            <div class="pt-2 relative">
                <h2 class="font-semibold text-l text-blue-800 leading-tight pl-2"> Skills </h2>
            </div>
        </div>

        <div class="px-2 pt-2 flex justify-start">

            <select id="skills_id" class="mt-1 w-4/5 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" name="skills_id" required wire:model.defer="skills_id" />
            <option value="" selected> Select a Skill </option>
            @foreach($lists as $list)
            <option value="{{$list->id}}"> {{$list->name}}</option>
            @endforeach
            </select>
            <button type="submit" class="w-1/6 ml-8 " wire:click="AddSkill()">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 20" fill="currentColor" class="text-gray-600 h-4 w-5 fill-current">
        ...
        <div class="ml-4 w-full md:w-full mb-1 mt-2" wire:poll.5000ms>
            ...
            <span class="px-2 inline-flex text-s leading-5 font-semibold rounded-full bg-green-600 text-white mt-2 transition duration-500 ease-in-out transform hover:-translate-y-1"> {{$skill->skills->name}} 
   ...
   <x-jet-confirmation-modal wire:model="confirm">
        ...
        <x-slot name="content">
            {{ __('Are you sure you want to remove this skill?') }}
        </x-slot>

        <x-slot name="footer">
            <x-jet-danger-button class="ml-2" wire:click="DeleteSkill( {{$confirm}})" wire:loading.attr="disabled">
              {{ __('Remove') }}
            </x-jet-danger-button>
