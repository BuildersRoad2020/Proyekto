 ...
 <x-jet-dialog-modal wire:model="confirmingTechnicianAdd">
            ...
            <x-slot name="content">
                ...
                <div class="mt-4">
                    ...
                    <option value="{{$specifiedcontractors->id}}" selected> {{$specifiedcontractors->name}} </option>
        ...
        <table class="border-collapse w-full">
         ...
         <x-jet-dialog-modal wire:model="confirmingTechnicianDelete">
            <x-slot name="title">
                {{ __('Delete User') }}
            </x-slot>

            <x-slot name="content">
                {{ __('Are you sure you want to delete this Technician? This will remove associated accounts') }}
            </x-slot>
    ...
    <!--Delete technician Modal -->
    <x-jet-dialog-modal wire:model="confirmingtechnicianDeletion">
        <x-slot name="title">
            {{ __('Delete technician') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Are you sure you want to delete this technician?') }}
        </x-slot>

        <x-slot name="footer">

            <x-jet-danger-button class="ml-2" wire:click="Deletetechnician()" wire:loading.attr="disabled">
                {{ __('Delete Account') }}
            </x-jet-danger-button>
            <x-jet-secondary-button wire:click="$set('confirmingtechnicianDeletion', false)" wire:loading.attr="disabled">
                {{ __('Nevermind') }}
            </x-jet-secondary-button>
